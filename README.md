...
The recommended way to add `DynamsoftMRZScannerBundle` to your app is via the platform's standard package manager — you do **not** need to clone or include the source from this repository. Cloning the repo is only necessary if you intend to customize the bundle's internals (see [Further Customization](#further-customization)).
